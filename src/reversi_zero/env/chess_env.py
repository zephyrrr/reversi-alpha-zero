...
class ChessEnv(BaseEnv):
    ...
    def update(self, board_data, next_player=Player.black):
        ...
        self.turn = self.board.board.fullmove_number
        self.resigned = False
        return self
    ...
    def step(self, action):
        ...
        self.board.board.push_uci(Board.labels[action])
    ...
    def legal_moves(self):
        self.move_lookup = {k:v for k,v in zip((chess.Move.from_uci(move) for move in Board.labels),range(len(Board.labels)))}
...
class Board():
    ...
    def board_to_string(black, white):
        chess_board = chess.Board(black)
        return str(chess_board)
